...
LaTeX 渲染预览。**判分/共识/诊断内核零改动；纯前端。**

- **① 输入框不再「过深看不清」**：根因是 `.chatbox-textarea` 等**写死** `background: rgba(0,0,0,.25)`——不随
  主题翻面，浅色下成了「深底 + 深字」不可读。新增语义 token `--field-bg`（深色 = 白微调亮面，浅色 = 淡色浅
  面），把 **10 处**写死的 `rgba(0,0,0,.2/.25)` 输入/内嵌面板（textarea、校对编辑器、各 select、结果框、公式框
  …）统一改用 `--field-bg`；`.pc-select` 写死的浅灰字 `#e5e7eb` 也改 `var(--text-main)`。代码块（0.3/0.35）保持
  深底不动。
- **② 彻底去边框**：`--glass-border` 由淡灰改为 **`transparent`**（深/浅两主题）——所有卡片/面板/气泡/分隔线
  边框隐去，靠淡填充 + 间距分区，不再「卡片味」。为保可用性：表单控件用 `--field-bg` 填充仍可辨；`.btn-secondary`
  由「透明底 + 边框」改为「`--field-bg` 底」以免无边框后隐形；AI 气泡填充略提（0.04→0.07）。焦点态仍显示强调色
  描边。
- **③ Q&A 输入实时 LaTeX 预览（`ChatBox`）**：输入框下方新增「预览」块，用与**校对屏同一渲染器**
  （`renderMarkdownMath`/`renderSource` + `useMathJax`）实时把草稿渲染成 md+latex，发送前即可见公式效果；纯文本
  模式（renderMode 3）不显示预览。助手屏与答疑屏共用 `ChatBox`，两屏同得此能力。
- **验收（运行—观测）**：`tsc --noEmit` 0 错、`vite build` 成功；**headless Chrome 两张截图**（临时落到答疑屏并
  预填 LaTeX，截完还原）确认深/浅两态：输入框浅底可读、面板无边框、「预览」块正确 typeset 公式。
